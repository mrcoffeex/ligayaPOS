<?php  
    ...
    if (isset($_POST['product_search'])) {
        ...
        if ($count_results <= 0) {
            header("location: pullout_counter?note=not_found");
        }else{
            ...
            //check if duplicate
            $duplicate_check=$link->query("Select * From `gy_pullout` Where `gy_product_code`='$my_product_code' AND `gy_pullout_code`='$my_trans_code' AND `gy_pullout_by`='$user_id'");
            $count_duplicate=$duplicate_check->num_rows;

            if ($count_duplicate > 0) {
                header("location: pullout_counter?note=duplicate");
            }else{
                //insert to database
                $insert_data=$link->query("Insert Into `gy_pullout`(`gy_pullout_code`,`gy_product_code`, `gy_product_name`, `gy_pullout_type`, `gy_pullout_note`, `gy_pullout_quantity`, `gy_pullout_date`, `gy_pullout_status`, `gy_backorder_status`, `gy_pullout_by`) Values('$my_trans_code','$my_product_code','$my_product_name','','', 0,'$date_now', 0, 0,'$user_id')");

                if ($insert_data) {
                    header("location: pullout_counter?note=nice");
