<?php  
    ...
    if (isset($_POST['submit_edit'])) {
        ...
        $my_dir_value_s = words($_GET['dir']);

        $myorigprice = words($_POST['my_retail_price']);
        $my_quantity = words($_POST['my_quantity']);
        $my_pin_edit = words($_POST['my_pin_edit']);
        $my_retail_price = words($_POST['my_retail_price']);

        //get the srp
        $get_product_info=$link->query("Select * From `gy_products` LEFT JOIN `gy_trans_details` On `gy_products`.`gy_product_id`=`gy_trans_details`.`gy_product_id` Where `gy_trans_details`.`gy_transdet_id`='$my_dir_value'");
        ...
        if ($my_pin_edit == "") {
            //Update Discount
            $update_data=$link->query("Update `gy_trans_details` SET `gy_trans_quantity`='$my_quantity',`gy_trans_ref_rep_quantity`='$my_quantity',`gy_product_price`='$my_retail_price',`gy_product_origprice`='$myorigprice',`gy_product_discount`='$my_discount' Where `gy_transdet_id`='$my_dir_value'");
            ...
            if ($count_res > 0) {
                ...
                header("location: cashier?note=pin_out&cd=$my_dir_value_s");
            }
        }        
    }
?>