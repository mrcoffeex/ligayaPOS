<?php
    ...
    if ($search_text == "mrcoffeex_only_space") {
        ...
        $my_project_header_title = "Pull-Out Summary Report by <span style='color: blue;'>".date("F d, Y", strtotime($search_text))."</span>";

        $my_query=$link->query("Select * From `gy_pullout` Where date(`gy_pullout_date`)='$search_text' AND `gy_pullout_by`='$user_id' Order By `gy_pullout_date` DESC");
