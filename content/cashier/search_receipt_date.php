<?php
    ...
    if ($search_text == "mrcoffeex_only_space") {
        ...
        $my_project_header_title = "Search Transaction: ".date("F d, Y", strtotime($search_text));

        //count results
        $count_search_results=$link->query("Select * From `gy_transaction` Where date(`gy_trans_date`)='$search_text' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0'");
        $search_count=$count_search_results->num_rows;

        $query_one = "Select * From `gy_transaction` Where date(`gy_trans_date`)='$search_text' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC";

        $query_two = "Select COUNT(`gy_trans_id`) From `gy_transaction` Where date(`gy_trans_date`)='$search_text' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC";

        $query_three = "Select * From `gy_transaction` Where date(`gy_trans_date`)='$search_text' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC ";
