<?php
    ...
    try {
        ...
        $cashier = $user_info;
        $sales = $trans_details['gy_prepared_by'];
        ...
        $printer->feed();
        $printer->setEmphasis(true);
        $printer->text(addSpaces('', 12) . addSpaces('Royal Fee ', 10) . addSpaces($royal_fee, 8) . "\n");
        $printer->text(addSpaces('', 15) . addSpaces('Total ', 7) . addSpaces(@number_format($total,2), 8) . "\n");
        $printer->text(addSpaces('', 15) . addSpaces($method , 7) . addSpaces(@number_format($trans_details['gy_trans_cash'],2), 8) . "\n");
        $printer->text(addSpaces('', 15) . addSpaces('Change ', 7) . addSpaces(@number_format($my_change,2), 8) . "\n");
        $printer->setEmphasis(false);
        $printer->feed();
        $printer->setJustification(Printer::JUSTIFY_CENTER);
        ...
        $my_note_text = "Print Receipt Transaction Code: ".$transaction;
        my_notify($my_note_text,$user_info);
        header("location: sales");

    } catch (Exception $e) {
        header("location: sales?note=error_printer");
    }
