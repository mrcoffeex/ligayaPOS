<?php  
    ...
    if (isset($_POST['submit_discount'])) {
        ...
        $get_info=$link->query("Select * From `gy_tra_details` Where `gy_transdet_id`='$my_dir_value'");
        $info_row=$get_info->fetch_array();

        $my_product_code=$info_row['gy_product_code'];

        //check pin
        $check_pin=$link->query("Select * From `gy_optimum_secure` Where `gy_sec_value`='$my_pin'");
        ...
        if ($count_res > 0) {
            ...
            if ($my_discount <= $my_discount_limit) {
                //delete to database
                $update_data=$link->query("Update `gy_tra_details` SET `gy_product_discount`='$my_discount' Where `gy_transdet_id`='$my_dir_value'");
                ...
                if ($update_data) {
                    ...
                    header("location: tra_counter?note=add_discount");
                }else{
                    header("location: tra_counter?note=error");
                }
            }else{
                header("location: tra_counter?note=discount_limit");
            }

            
        }else{
            header("location: tra_counter?note=pin_error");
        }
