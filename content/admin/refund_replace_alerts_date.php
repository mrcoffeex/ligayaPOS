<?php
    ...
    if ($datef == $datet) {
        $my_project_header_title = "Refund/Replace Report: ".date("M d, Y", strtotime($search_text))."</span>";
    }else{
        $my_project_header_title = "Refund/Replace Report: ".date("M d", strtotime($datef))." - ".date("M d, Y", strtotime($datef));
    }

    $query_one = "Select * From `gy_refund` Where date(`gy_refund_date`) BETWEEN '$datef' AND '$datet' Order By `gy_refund_id` ASC";

    $query_two = "Select COUNT(`gy_refund_id`) From `gy_refund` Where date(`gy_refund_date`) BETWEEN '$datef' AND '$datet' Order By `gy_refund_id` ASC";

    $query_three = "Select * From `gy_refund` Where date(`gy_refund_date`) BETWEEN '$datef' AND '$datet' Order By `gy_refund_id` ASC ";
