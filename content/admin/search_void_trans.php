    ...
    if ($search_text == "mrcoffeex_only_space") {
         echo "
            <script>
                ...
                window.location.href = 'void_trans'
            </script>
         ";
        ...
        $my_project_header_title = "Search Void Transaction: ".$search_text;

        //count results
        $count_search_results=$link->query("Select * From `gy_void` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%'");
        $search_count=$count_search_results->num_rows;

        $query_one = "Select * From `gy_void` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' Order By `gy_trans_date` DESC";

        $query_two = "Select COUNT(`gy_void_id`) From `gy_void` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' Order By `gy_trans_date` DESC";

        $query_three = "Select * From `gy_void` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' Order By `gy_trans_date` DESC ";
