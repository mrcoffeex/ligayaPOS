<?php  
    ...
    if (isset($_POST['submit_edit'])) {
        ...
        //get the srp
        $get_product_info=$link->query("Select * From `gy_products` LEFT JOIN `gy_tra_details` On `gy_products`.`gy_product_code`=`gy_tra_details`.`gy_product_code` Where `gy_tra_details`.`gy_transdet_id`='$my_dir_value'");
        ...
        if ($my_pin_edit == "") {
            //Update Discount
            $update_data=$link->query("Update `gy_tra_details` SET `gy_trans_quantity`='$my_quantity', `gy_trans_ref_rep_quantity`='$my_quantity',`gy_product_price`='$my_retail_price',`gy_product_discount`='$my_discount' Where `gy_transdet_id`='$my_dir_value'");

            if ($update_data) {
                header("location: tra_counter?note=item_update");
            }else{
            ...
            if ($count_res > 0) {
                ...
                header("location: tra_counter?note=pin_out");
            }
        }        
    }
?>