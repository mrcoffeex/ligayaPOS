<?php  
    ...
    if (isset($_POST['my_secure_pin'])) {
    	...
    	if (encryptIt($my_secure_pin) == $get_values['gy_sec_value']) {
            ...
            if ($recover_items) {
                //delete query
                $update_back_order_data=$link->query("Update `gy_pullout` SET `gy_backorder_status`='0' Where `gy_pullout_id`='$my_dir_value'");

                if ($update_back_order_data) {
                    $my_note_text = "Back Order Item -> ".$pullout_row['gy_product_name']." unchecked due to wrong input";
                    my_notify($my_note_text,$user_info);
                    header("location: back_order_reports?note=unchecked");
                }else{
