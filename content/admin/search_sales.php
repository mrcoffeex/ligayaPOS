<?php
    ...
    if ($search_text == "mrcoffeex_only_space") {
        ...
        //count results
        $count_search_results=$link->query("Select * From `gy_transaction` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0'");
        $search_count=$count_search_results->num_rows;

        $query_one = "Select * From `gy_transaction` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC";

        $query_two = "Select COUNT(`gy_trans_id`) From `gy_transaction` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC";

        $query_three = "Select * From `gy_transaction` Where CONCAT(`gy_trans_code`,`gy_trans_custname`) LIKE '%$search_text%' AND `gy_trans_type`='1' AND `gy_trans_status`='1' AND `gy_user_id`>'0' Order By `gy_trans_date` DESC ";
