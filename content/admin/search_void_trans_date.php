<?php
    ...
    if ($datef == $datet) {
        $my_project_header_title = "Search Void Transaction: ".date("M d, Y", strtotime($datef));
    }else{
        $my_project_header_title = "Search Void Transaction: ".date("M d", strtotime($datef))." - ".date("M d, Y", strtotime($datet));
    }

    //count results
    $count_search_results=$link->query("Select * From `gy_void` Where date(`gy_trans_date`) BETWEEN '$datef' AND '$datet'");
    $search_count=$count_search_results->num_rows;

    $query_one = "Select * From `gy_void` Where date(`gy_trans_date`) BETWEEN '$datef' AND '$datet' Order By `gy_trans_date` DESC";

    $query_two = "Select COUNT(`gy_void_id`) From `gy_void` Where date(`gy_trans_date`) BETWEEN '$datef' AND '$datet' Order By `gy_trans_date` DESC";

    $query_three = "Select * From `gy_void` Where date(`gy_trans_date`) BETWEEN '$datef' AND '$datet' Order By `gy_trans_date` DESC ";
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Void Transaction Data Table <b><?php echo $search_count; ?></b> result(s)
                        </div>
                        <!-- /.panel-heading -->
