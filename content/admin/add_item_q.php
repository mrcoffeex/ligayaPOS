<?php  
    ...
    if (isset($_POST['product_search'])) {
        ...
        if ($count_item < 1) {
            header("location: edit_quotation?note=not_found&cd=$my_trans");
        }else if ($product_row['gy_product_quantity'] < 1) {
            header("location: edit_quotation?note=out_stock&cd=$my_trans");
        }else{
            ...
            if ($count_duplicate > 0) {
                header("location: edit_quotation?note=duplicate&cd=$my_trans");
            }else{
                //insert to database
                $insert_data=$link->query("Insert Into `gy_trans_details`(`gy_trans_code`,`gy_transdet_date`, `gy_product_code`, `gy_product_price`, `gy_product_discount`, `gy_trans_quantity`, `gy_transdet_type`) Values('$my_trans','$my_date','$my_code','$my_price','0','1','0')");
                    ...
                    if ($insert_data) {    
                        header("location: edit_quotation?note=nice&cd=$my_trans");
                                
                    }else{
                        header("location: edit_quotation?note=error&cd=$my_trans");
                    }
            }   
        }
    }
